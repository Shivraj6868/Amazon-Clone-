# 🛒 Amazon Clone Web Application


A frontend clone of Amazon's website homepage built using **HTML5 and CSS3**.

This project was created to practice real-world frontend development concepts like webpage structure, layouts, CSS styling, Flexbox, positioning, hover effects, and Git/GitHub workflow.


## 🌐 Live Demo

🚀 Website Link: https://shivraj6868.github.io/Amazon-Clone-/




---

# 📌 Project Preview


## 🏠 Homepage

<img width="1277" height="661" alt="image" src="https://github.com/user-attachments/assets/5ce676de-36b0-4b81-ae1d-6971c2afd0c2" />


## 🔍 Category Dropdown Menu

<img width="639" height="656" alt="image" src="https://github.com/user-attachments/assets/f9e64193-e25b-45ea-8932-f388cae4a25b" />



## 🧭 Navigation Bar

<img width="1279" height="99" alt="image" src="https://github.com/user-attachments/assets/3efa6033-bb29-4871-b614-0bdb35ccb242" />



## 📌 Footer Section

<img width="1278" height="661" alt="image" src="https://github.com/user-attachments/assets/19b62e8a-5c05-4bff-9fb9-bea29bcf76ee" />



---

# ✨ Features


- ✅ Amazon inspired UI design
- ✅ Navigation bar
- ✅ Search bar section
- ✅ Category dropdown menu
- ✅ Hero banner section
- ✅ Product category cards
- ✅ Footer section
- ✅ Hover effects
- ✅ CSS positioning
- ✅ Responsive layout structure
- ✅ Git tracked development workflow



---

# 🛠️ Technologies Used


## Frontend

- HTML5
- CSS3


## Tools Used

- Visual Studio Code
- Git
- GitHub
- GitHub Pages



---

# 📂 Project Structure


